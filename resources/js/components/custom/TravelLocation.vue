<script setup>
const props = defineProps(['name', 'price', 'title', 'length', 'image']);
</script>
...
<template>
    <article class="mx-auto pb-5 max-w-sm transform duration-500 hover:-translate-y-1 cursor-pointer group">
        ...
        <div class="flex justify-between my-5 ">
            <div class="text-orange-500 text-base font-semibold">{{name}}</div>
            <div class="text-base text-right"><span class="font-bold">{{price}}</span>/Day</div>
        </div>
        <h2 class="font-bold text-2xl">{{ title }}</h2>
        <div class="flex justify-between items-center mt-3 ">
            <div class="text-base uppercase font-semibold text-gray-500">{{length}} Days Tour</div>
